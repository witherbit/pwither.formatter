...
namespace pwither.formatter
{
    internal sealed class BinaryAssemblyInfo
    {
        ...
        internal Assembly GetAssembly()
        {
            if (_assembly == null)
            {
                _assembly = BitFormatterServices.LoadAssemblyFromStringNoThrow(_assemblyString);
                if (_assembly == null)
                {
                    throw new BitSerializationException(SR.Format(SR.Serialization_AssemblyNotFound, _assemblyString));
                }
            }
            return _assembly;
        }
