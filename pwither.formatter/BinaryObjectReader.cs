...
namespace pwither.formatter
{
    internal sealed class ObjectReader
    {
        ...
        internal object Deserialize(BinaryParser serParser)
        {
            ...
            _isSimpleAssembly = (_formatterEnums._assemblyFormat == BitFormatterAssemblyStyle.Simple);
            ...
            if (TopObject == null)
            {
                throw new BitSerializationException(SR.Serialization_TopObject);
            }
        ...
        private void CheckSerializable(Type t)
        {
            ...
            {
                throw new BitSerializationException(SR.Format(CultureInfo.InvariantCulture, SR.Serialization_NonSerType, t.FullName!, t.Assembly.FullName!));
            }
        }
        ...
        internal void Parse(ParseRecord pr)
        {
            ...
            {
                case BitInternalParseTypeE.SerializedStreamHeader:
                    ParseSerializedStreamHeader(pr);
                    break;
                case BitInternalParseTypeE.SerializedStreamHeaderEnd:
                    ParseSerializedStreamHeaderEnd(pr);
                    break;
                case BitInternalParseTypeE.Object:
                    ParseObject(pr);
                    break;
                case BitInternalParseTypeE.ObjectEnd:
                    ParseObjectEnd(pr);
                    break;
                case BitInternalParseTypeE.Member:
                    ParseMember(pr);
                    break;
                case BitInternalParseTypeE.MemberEnd:
                    ParseMemberEnd(pr);
                    break;
                case BitInternalParseTypeE.Body:
                case BitInternalParseTypeE.BodyEnd:
                case BitInternalParseTypeE.Envelope:
                case BitInternalParseTypeE.EnvelopeEnd:
                    break;
                case BitInternalParseTypeE.Empty:
                default:
                    throw new BitSerializationException(SR.Format(SR.Serialization_XMLElement, pr._name));
            }
        }
        ...
        {
            throw new BitSerializationException(SR.Format(SR.Serialization_ParseError, onStack._name + " " + onStack._parseTypeEnum + " " + processing._name + " " + processing._parseTypeEnum));
        }
        ...
        private void ParseObject(ParseRecord pr)
        {
            ...
            if (pr._objectPositionEnum == BitInternalObjectPositionE.Top)
            {
                _topId = pr._objectId;
            }

            if (pr._parseTypeEnum == BitInternalParseTypeE.Object)
            {
                _stack!.Push(pr); // Nested objects member names are already on stack
            }

            if (pr._objectTypeEnum == BitInternalObjectTypeE.Array)
            {
                ParseArray(pr);
                return;
            }
            ...
            else
            {
                ...
                pr._newObj = BitFormatterServices.GetUninitializedObject(pr._dtType);
            ...
            if (pr._newObj == null)
            {
                throw new BitSerializationException(SR.Format(SR.Serialization_TopObjectInstantiate, pr._dtType));
            }
        ...
        private void ParseObjectEnd(ParseRecord pr)
        {
            ...
            if (objectPr._objectPositionEnum == BitInternalObjectPositionE.Top)
            {
            ...
            if (objectPr._objectTypeEnum == BitInternalObjectTypeE.Array)
            {
                if (objectPr._objectPositionEnum == BitInternalObjectPositionE.Top)
                {
                    TopObject = objectPr._newObj;
        ...
        {
            Debug.Assert(_stack != null);
            if (pr._arrayTypeEnum == BitInternalArrayTypeE.Base64)
            {
            ...
            else if ((pr._arrayTypeEnum == BitInternalArrayTypeE.Jagged) || (pr._arrayTypeEnum == BitInternalArrayTypeE.Single))
            {
                ...
                if (pr._arrayTypeEnum == BitInternalArrayTypeE.Single)
                {
            ...
            else if (pr._arrayTypeEnum == BitInternalArrayTypeE.Rectangular)
            {
                // Rectangle array
            ...
            {
                throw new BitSerializationException(SR.Format(SR.Serialization_ArrayType, pr._arrayTypeEnum));
            }
        }
        ...
        private void ParseArrayMember(ParseRecord pr)
        {
            ...
            if (objectPr._arrayTypeEnum == BitInternalArrayTypeE.Rectangular)
            {
            ...
            if (pr._memberValueEnum == BitInternalMemberValueE.Reference)
            {
                // Object Reference
            ...
            else if (pr._memberValueEnum == BitInternalMemberValueE.Nested)
            {
                ...
                {
                    Debug.Assert(objectPr._newObj != null);
                    if ((objectPr._arrayElementType.IsValueType) && (pr._arrayElementTypeCode == BitInternalPrimitiveTypeE.Invalid))
                    {
            ...
            else if (pr._memberValueEnum == BitInternalMemberValueE.InlineValue)
            {
                ...
                else if (objectPr._isArrayVariant)
                {
                    ...
                    if (pr._keyDt == null)
                    {
                        throw new BitSerializationException(SR.Serialization_ArrayTypeObject);
                    }
            ...
            else if (pr._memberValueEnum == BitInternalMemberValueE.Null)
            {
        ...
        private void ParseArrayMemberEnd(ParseRecord pr)
        {
            ...
            if (pr._memberValueEnum == BitInternalMemberValueE.Nested)
            {
                ParseObjectEnd(pr);
            }
        }
        ...
        private void ParseMember(ParseRecord pr)
        {
            ...
            {
                case BitInternalMemberTypeE.Item:
                    ParseArrayMember(pr);
                    return;
                case BitInternalMemberTypeE.Field:
                    break;
            }
            ...
            if (pr._memberValueEnum == BitInternalMemberValueE.Null)
            {
                // Value is Null
            ...
            else if (pr._memberValueEnum == BitInternalMemberValueE.Reference)
            {
                Debug.Assert(_objectManager != null);
                // See if object has already been instantiated
                object? refObj = _objectManager.GetObject(pr._idRef);
            ...
            else if (pr._memberValueEnum == BitInternalMemberValueE.InlineValue)
            {
                ...
                else if (pr._dtTypeCode == BitInternalPrimitiveTypeE.Invalid)
                {
                    ...
                    {
                        throw new BitSerializationException(SR.Format(SR.Serialization_TypeMissing, pr._name));
                    }
                    else
                    {
        ...
        internal Type? FastBindToType(string? assemblyName, string typeName)
        {
            ...
            if (entry == null || entry.AssemblyName != assemblyName)
            {
                ...
                {
                    type = BitFormatterServices.GetTypeFromAssembly(assm, typeName);
                }
        ...
        internal Type? GetType(BinaryAssemblyInfo assemblyInfo, string name)
        {
            ...
            else
            {
                ...
                if (objectType == null)
                {
                    ...
                    {
                        objectType = BitFormatterServices.GetTypeFromAssembly(sourceAssembly, name);
                    }
