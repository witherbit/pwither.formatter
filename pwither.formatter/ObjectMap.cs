...
namespace pwither.formatter
{
    ...
    internal sealed class ObjectMap
    {
            ...
            BinaryAssemblyInfo assemblyInfo)
        {
            ...
            _binaryTypeEnumA = new BitBinaryTypeEnum[_memberTypes.Length];
            _typeInformationA = new object[_memberTypes.Length];

            for (int i = 0; i < _memberTypes.Length; i++)
            {
                BitBinaryTypeEnum binaryTypeEnum = BinaryTypeConverter.GetParserBinaryTypeInfo(_memberTypes[i]!, out object? typeInformation, objectReader._context);
        ...
        internal ObjectMap(string objectName, string[] memberNames, BitBinaryTypeEnum[] binaryTypeEnumA, object?[] typeInformationA, int[] memberAssemIds, ObjectReader objectReader, int objectId, BinaryAssemblyInfo assemblyInfo, SizedArray assemIdToAssemblyTable)
        {
            _objectName = objectName;
            _memberNames = memberNames;
            ...
            {
                throw new BitSerializationException(SR.Format(SR.Serialization_Assembly, objectName));
            }
        ...
        [RequiresUnreferencedCode("Types might be removed")]
        internal static ObjectMap Create(
            string name, string[] memberNames, BitBinaryTypeEnum[] binaryTypeEnumA, object?[] typeInformationA,
