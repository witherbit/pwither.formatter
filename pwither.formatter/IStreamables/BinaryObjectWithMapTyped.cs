...
namespace pwither.formatter
{
    ...
    {
        internal BitBinaryHeaderEnum _binaryHeaderEnum;
        ...
        internal BinaryObjectWithMapTyped(BitBinaryHeaderEnum binaryHeaderEnum)
        {
            _binaryHeaderEnum = binaryHeaderEnum;
        }

        internal void Set(int objectId, string name, int numMembers, string[] memberNames, BitBinaryTypeEnum[] binaryTypeEnumA, object?[] typeInformationA, int[] memberAssemIds, int assemId)
        {
            _objectId = objectId;
            _assemId = assemId;
            _name = name;
            ...
            _binaryHeaderEnum = assemId > 0 ?
                 BitBinaryHeaderEnum.ObjectWithMapTypedAssemId :
                 BitBinaryHeaderEnum.ObjectWithMapTyped;
        }
        ...
        public void Read(BinaryParser input)
        {
            ...
            _binaryTypeEnumA = new BitBinaryTypeEnum[_numMembers];
            ...
            for (int i = 0; i < _numMembers; i++)
            {
                _binaryTypeEnumA[i] = (BitBinaryTypeEnum)input.ReadByte();
            }
            for (int i = 0; i < _numMembers; i++)
            {
                if (_binaryTypeEnumA[i] != BitBinaryTypeEnum.ObjectUrt && _binaryTypeEnumA[i] != BitBinaryTypeEnum.ObjectUser)
                {
            ...
            if (_binaryHeaderEnum == BitBinaryHeaderEnum.ObjectWithMapTypedAssemId)
            {
